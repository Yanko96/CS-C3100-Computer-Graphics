...
Curve evalBezier(const vector<Vec3f>& P, unsigned steps, bool adaptive, float errorbound, float minstep) {
	...
	for (unsigned i = 0; i < (P.size() - 1) / 3; i++) {
		Curve frac = coreBezier(P[3 * i], P[3 * i + 1], P[3 * i + 2], P[3 * i + 3], Vec3f(0, 0, 0), steps);
		R.insert(R.end(), frac.begin(), frac.end());
	}
	// EXTRA CREDIT NOTE:
...
Curve evalBspline(const vector<Vec3f>& P, unsigned steps, bool adaptive, float errorbound, float minstep) {
	...
	for (unsigned i = 0; i < (P.size() - 4) + 1; ++i) {
		Mat4f bSpline_Control_Points;
		Mat4f bezier_Control_Points;
		Vec3f Control_Point1, Control_Point2, Control_Point3, Control_Point4;

		bSpline_Control_Points.setRow(0, Vec4f(P[i].x, P[i + 1].x, P[i + 2].x, P[i + 3].x));
		bSpline_Control_Points.setRow(1, Vec4f(P[i].y, P[i + 1].y, P[i + 2].y, P[i + 3].y));
		bSpline_Control_Points.setRow(2, Vec4f(P[i].z, P[i + 1].z, P[i + 2].z, P[i + 3].z));
		bSpline_Control_Points.setRow(3, Vec4f(0, 0, 0, 1));

		bezier_Control_Points = bSpline_Control_Points * bBSpline * bBazier.inverted();

		Control_Point1 = Vec3f(bezier_Control_Points(0, 0), bezier_Control_Points(1, 0), bezier_Control_Points(2, 0));
		Control_Point2 = Vec3f(bezier_Control_Points(0, 1), bezier_Control_Points(1, 1), bezier_Control_Points(2, 1));
		Control_Point3 = Vec3f(bezier_Control_Points(0, 2), bezier_Control_Points(1, 2), bezier_Control_Points(2, 2));
		Control_Point4 = Vec3f(bezier_Control_Points(0, 3), bezier_Control_Points(1, 3), bezier_Control_Points(2, 3));
		auto frac = coreBezier(Control_Point1, Control_Point2, Control_Point3, Control_Point4, Vec3f(0, 0, 0), steps);
		R.insert(R.end(), frac.begin(), frac.end());
